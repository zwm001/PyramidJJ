...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        data = self.getpq(self.fetch(url, headers=self.headers, params=params))
        result = {}
        if tid in ['makers', 'genres']:
    ...
    def detailContent(self, ids):
        ...
        for i in c.items():
            if re.search(r"导演:|女优:", i.text()):
                ac.extend(
                    ['[a=cr:' + json.dumps({'id': j.attr('href'), 'name': j.text()}) + '/]' + j.text() + '[/a]' for j in
                     i('a').items()])
            elif '发行商:' in i.text():
                dt.extend(
                    ['[a=cr:' + json.dumps({'id': j.attr('href'), 'name': j.text()}) + '/]' + j.text() + '[/a]' for j in
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if '相关' in flag:
            try:
                v = self.getpq(self.fetch(id, headers=self.headers))
                sctx = v('body script').text()
                urls = self.execute_js(sctx)
